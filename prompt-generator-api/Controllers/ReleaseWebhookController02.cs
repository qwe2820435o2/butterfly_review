...
public class ReleaseWebhookController02 : ControllerBase
{
    ...
    {
        _logger.LogInformation("收到 Release02 webhook 请求: Method={Method}, Path={Path}, QueryString={QueryString}",
            Request.Method,
            Request.Path,
            Request.QueryString);
        ...
        {
            _logger.LogError(ex, "Release02 读取表单数据时发生错误");
        }
        ...
        if (!string.IsNullOrWhiteSpace(rawRequestJson))
        {
            _ = Task.Run(async () =>
            {
                ...
                {
                    _logger.LogError(ex, "Release02 处理 webhook 数据时发生错误");
                }
            });
        }
        else
        {
            _logger.LogWarning("Release02 rawRequest 字段缺失，跳过异步处理");
        }
    ...
    private async Task ProcessWebhookAsync(string rawRequestJson)
    {
        ...
        try
        {
            _logger.LogInformation("Release02 开始处理 Release02 webhook 数据，rawRequest 长度: {Length}, Timestamp: {Timestamp} , RawRequestJson: {RawRequestJson}", 
            ...
            {
                _logger.LogError("Release02 解析 rawRequest JSON 失败，返回 null. Timestamp: {Timestamp}", timestamp);
                return;
            }

            _logger.LogInformation("Release02 解析的请求数据: TagNumber={TagNumber}, HasDate={HasDate}, HasEmail={HasEmail}",
            ...
            {
                _logger.LogWarning("Release02 TagNumber 为空，无法执行 Upsert，跳过保存. SubmissionId: {SubmissionId}, Timestamp: {Timestamp}", 
            ...
            _logger.LogInformation("Release02 成功保存/更新 Release submission 到 MongoDB, SubmissionId: {SubmissionId}, TagNumber: {TagNumber}, Timestamp: {Timestamp}", 
                releaseSubmission.SubmissionId, 
        ...
        {
            _logger.LogError(ex, "Release02 解析 rawRequest JSON 时发生错误: {Message}. Timestamp: {Timestamp}", 
                ex.Message, 
                timestamp);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Release02 处理 webhook 数据时发生未预期的错误: {Message}. Timestamp: {Timestamp}", 
                ex.Message, 
                timestamp);
        }
    }
