...
public class YearInReviewController : ControllerBase
{
    ...
    public async Task<IActionResult> GetYearInReview(int year)
    {
        try
        {
            ...
            var yearInReview = new YearInReviewDto
            {
                ...
                MonthlyStats = CalculateMonthlyStatistics(releases, sightings, year),
                GeographicDistribution = CalculateGeographicDistribution(releases, sightings),
                TopContributors = CalculateTopContributors(sightings, limit: 10),
                Achievements = CalculateAchievements(releases, sightings)
            };
    ...
    /// </summary>
    private List<MonthlyStatisticsDto> CalculateMonthlyStatistics(
        IReadOnlyList<Models.Entities.ReleaseSubmission> releases,
        IReadOnlyList<Models.Entities.SightingSubmission> sightings,
        int year)
    {
        ...
        {
            var monthReleases = releases.Where(r =>
                r.ReleaseDateTimeUtc.HasValue &&
                r.ReleaseDateTimeUtc.Value.Year == year &&
                r.ReleaseDateTimeUtc.Value.Month == month).ToList();

            var monthSightings = sightings.Where(s =>
                s.SightingDateTimeUtc.HasValue &&
                s.SightingDateTimeUtc.Value.Year == year &&
                s.SightingDateTimeUtc.Value.Month == month).ToList();

            var uniqueTagNumbers = monthSightings.Select(s => s.TagNumber).Distinct().Count();

            monthlyStats.Add(new MonthlyStatisticsDto
            {
                Month = month,
                MonthName = monthNames[month - 1],
                Releases = monthReleases.Count,
                Sightings = monthSightings.Count,
                UniqueTagNumbersSighted = uniqueTagNumbers
            });
        }
    ...
    /// </summary>
    private List<ContributorDto> CalculateTopContributors(
        IReadOnlyList<Models.Entities.SightingSubmission> sightings,
        int limit = 10)
    {
        var contributorGroups = sightings
            .Where(s => !string.IsNullOrWhiteSpace(s.Email))
            .GroupBy(s => s.Email!.ToLowerInvariant())
            .Select(g => new ContributorDto
            {
                Email = g.Key, // For privacy, you might want to mask this
                SightingCount = g.Count(),
                UniqueTagNumbersReported = g.Select(s => s.TagNumber).Distinct().Count()
            })
            .OrderByDescending(c => c.SightingCount)
            .ThenByDescending(c => c.UniqueTagNumbersReported)
            .Take(limit)
            .ToList();

        // Assign ranks
        for (int i = 0; i < contributorGroups.Count; i++)
        {
            contributorGroups[i].Rank = i + 1;
        }
    ...
    /// </summary>
    private AchievementsDto CalculateAchievements(
        IReadOnlyList<Models.Entities.ReleaseSubmission> releases,
        IReadOnlyList<Models.Entities.SightingSubmission> sightings)
    {
        var tagNumbers = releases.Select(r => r.TagNumber).Distinct().ToList();
        ...
        FlightRecordDto? longestFlight = null;
        SurvivalRecordDto? longestSurvival = null;
        MostSightedDto? mostSighted = null;
        FarthestSightingDto? farthestSighting = null;

        double maxDistance = 0;
        int maxSurvivalDays = 0;
        int maxSightingCount = 0;
        double maxFarthestDistance = 0;
        ...
        foreach (var tagNumber in tagNumbers)
        {
            ...
            if (tagReleases.Count == 0 || tagSightings.Count == 0) continue;
            ...
            if (latestRelease.Latitude.HasValue && latestRelease.Longitude.HasValue)
            {
                ...
                foreach (var sighting in tagSightings)
                {
                    if (sighting.Latitude.HasValue && sighting.Longitude.HasValue)
                    {
                        ...
                        // Check for farthest sighting
                        if (distance > maxFarthestDistance)
                        {
                            maxFarthestDistance = distance;
                            farthestSighting = new FarthestSightingDto
                            {
                                TagNumber = tagNumber,
                                DistanceKm = distance,
                                ReleaseLocation = new LocationPointDto
                                {
                                    Latitude = latestRelease.Latitude.Value,
                                    Longitude = latestRelease.Longitude.Value,
                                    Address = latestRelease.Address,
                                    Date = latestRelease.ReleaseDateTimeUtc
                                },
                                SightingLocation = new LocationPointDto
                                {
                                    Latitude = sighting.Latitude.Value,
                                    Longitude = sighting.Longitude.Value,
                                    Address = sighting.Address,
                                    Date = sighting.SightingDateTimeUtc
                                },
                                SightingDate = sighting.SightingDateTimeUtc
                            };
                        }
                    }
                }
                ...
                if (totalDistance > maxDistance)
                {
                    ...
                    longestFlight = new FlightRecordDto
                    {
                        TagNumber = tagNumber,
                        TotalDistanceKm = totalDistance,
                        SightingCount = tagSightings.Count,
                        SurvivalDays = latestRelease.ReleaseDateTimeUtc.HasValue && lastSighting.SightingDateTimeUtc.HasValue
                            ? (int)(lastSighting.SightingDateTimeUtc.Value - latestRelease.ReleaseDateTimeUtc.Value).TotalDays
                            : null,
                        ReleaseDate = latestRelease.ReleaseDateTimeUtc,
            ...
            {
                var days = (int)(lastSighting.SightingDateTimeUtc.Value - latestRelease.ReleaseDateTimeUtc.Value).TotalDays;
                if (days > maxSurvivalDays)
                {
                    maxSurvivalDays = days;
                    longestSurvival = new SurvivalRecordDto
                    {
                        TagNumber = tagNumber,
                        SurvivalDays = days,
                        ReleaseDate = latestRelease.ReleaseDateTimeUtc,
                        LastSightingDate = lastSighting.SightingDateTimeUtc,
                        Status = lastSighting.DeadOrAlive
                    };
                }
            }

            // Check for most sighted
            if (tagSightings.Count > maxSightingCount)
            {
                maxSightingCount = tagSightings.Count;
                mostSighted = new MostSightedDto
                {
                    TagNumber = tagNumber,
                    SightingCount = tagSightings.Count,
                    ReleaseDate = latestRelease.ReleaseDateTimeUtc,
                    FirstSightingDate = tagSightings.First().SightingDateTimeUtc,
                    LastSightingDate = lastSighting.SightingDateTimeUtc
                };
            }
        }
